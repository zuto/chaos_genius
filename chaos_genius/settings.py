...
SECRET_KEY = os.getenv(
    "SECRET_KEY", default="t8GIEp8hWmR8y6VLqd6qQCMXzjRaKsx8nRruWNtFuec="
)
...
CHAOSGENIUS_VERSION_MAIN = os.getenv("CHAOSGENIUS_VERSION_MAIN", "0.11.0")
...
if CHAOSGENIUS_VERSION_POSTFIX == "git":
    CHAOSGENIUS_VERSION_POSTFIX += "-" + (
        latest_git_commit_hash() or "unknown"
    )

CHAOSGENIUS_VERSION = (
    CHAOSGENIUS_VERSION_MAIN + "-" + CHAOSGENIUS_VERSION_POSTFIX
)
"""ChaosGenius full version string"""

CHAOSGENIUS_ENTERPRISE_EDITION_KEY = os.getenv(
...
SOURCE_GOOGLE_SHEETS = _make_bool(
    os.getenv("SOURCE_GOOGLE_SHEETS", default=True)
)
...
SOURCE_FACEBOOK_ADS = _make_bool(
    os.getenv("SOURCE_FACEBOOK_ADS", default=False)
)
